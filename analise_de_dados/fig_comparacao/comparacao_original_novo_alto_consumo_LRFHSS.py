...
for folder in folders:
    ...
    for file in files:
        ...
        sim = pd.read_csv(os.path.join(path, folder, file), names=['timestamp','id','dist','elev','SF','status','a','b','c','d', 'num'])
...
soma_completo = 2.571*(statistics_completo['percentage_std']/np.sqrt(6))
...
for satelite in statistics_individual['sat'].unique():
    ...
    soma_individual = 2.571*(subset['percentage_std']/np.sqrt(6))
    plt.plot(subset['num_nodes'], subset['percentage_mean'], label=f'Satélite {satelite}', ls='--')
    plt.fill_between(subset['num_nodes'], subset['percentage_mean'], subset['percentage_mean'], alpha= 0.3)
...
for folder in folders:
    ...
    for file in files:
        ...
        sim = pd.read_csv(os.path.join(path, folder, file), names=['timestamp','id','dist','elev', 'SF','status','a','c','d','e','num'])
        sim = sim[sim['timestamp'] <= 900]
